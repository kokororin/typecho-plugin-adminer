...
class Adminer {
	...
	*/
	function head() {
		?>
<link rel="stylesheet" type="text/css" href="<?php echo TYPECHO_ADMINER_STATIC_PREFIX;?>/jush.css">
<?php
		return true;
	}
...
<tr><th><?php echo lang('Server'); ?><td><input name="auth[server]" value="<?php echo TYPECHO_ADMINER_HOST; ?>" title="hostname[:port]" placeholder="localhost" autocapitalize="off" readonly>
<tr><th><?php echo lang('Username'); ?><td><input name="auth[username]" id="username" value="<?php echo TYPECHO_ADMINER_USER; ?>" autocapitalize="off" readonly>
<tr><th><?php echo lang('Password'); ?><td><input type="password" name="auth[password]" value="<?php echo TYPECHO_ADMINER_PWD;?>" readonly>
...
<?php
		if ($missing == "auth") {
			...
			if (support("sql")) {
				?>
<script type="text/javascript" src="<?php echo TYPECHO_ADMINER_STATIC_PREFIX;?>/jush.js"></script>
<script type="text/javascript">
<?php
				if ($tables) {
					$links = array();
